...
def extract_review_files():
    ...
    filels = ["SHADOW", "PASSWD", "GROUP", "PROFILE", "HOSTS"]
...
def os_config_files_review(playbook):
    ...
    for each_host_event in r.events:
        ...
        if each_host_event["event"] == "runner_on_ok":
            ...
            if (
                each_host_event["event_data"]["task"] != "Gathering Facts"
                and each_host_event["event_data"]["task"] != "include_vars"
            ):
                # print(each_host_event['event_data']['task'])
                taskls.append(each_host_event["event_data"]["task"])
                if each_host_event["event_data"]["task"] != "debug":
                    ...
                    outputstr = "debug: " + each_host_event["event_data"]["res"]["msg"]
                    outputls.append(outputstr)
